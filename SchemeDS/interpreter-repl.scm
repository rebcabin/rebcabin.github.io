...
;; The following are defined in the host Scheme environment, to support use by ds:wrong
;; Note LAML can't handle global names with characters that aren't allowed in filenames, 
;; like * and <.  Wanted to use *current-failure-continuation* below.
...
(define ds:wrong
  (lambda (x)
    (display x)  ; allows display before store is supplied, in case of mismatches
    (newline)
    (lambda (s)
      (((ds:applicate failure-continuation-value (ds:sequence)) #f) s)))) ;; #f is for unused continuation

;; Called from non-DS procedures when execution context can't easily support ds:wrong.
...
(define ds:wrong-wrong
  (lambda (x)
    ...
    ;; return uninitialized location; bogus, since not associated with an address
    (ds:sequence ds:unspecified ds:false))) 


;; The following is the first code evaluated by the interpreter itself.  
;; This code defines the interpreter's repl, the 'load' procedure, and 
;; then loads subsequent definitions from prelude.scm.
(define bootstrap-source
  '(begin
      ;; a parameterizerable repl - see chatty-repl in prelude.scm for example of use
                    ...
                    (begin
                      (call-with-current-continuation
                       (lambda (c)
                         ...
                         (display-proc (eval p (interaction-environment)))))
                      (reploop))))))
