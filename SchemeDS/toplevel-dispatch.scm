...
(define (expression-meaning-toplevel E)
  (if (not (pair? E))
      ...
      (case (ds:first E)
        ((begin)  ; top-level begin logic was adapated from lambda command sequence processing
        ...
        ((define) (expression-guard-min E 3
                   (lambda ()
                     (if (pair? (cadr E))
                         (expression-meaning-procedure-definition (ds:second E) (cddr E))
                          (expression-guard E 3
                           (lambda ()
                             (expression-meaning-toplevel-definition (ds:second E) (ds:third E))))))))
        (else     (expression-meaning E)))))
